...
export const Languages = observer(() => {
    ...
    const { filters, enableFilter, disableFilter } = settingsStore;
    ...
    const languagesFilters = filters.filter(
        (filter: Filter) => filter.groupId === FiltersGroupId.LANGUAGES || filter.groupId === FiltersGroupId.INTEGRATED,
    );

    const integratedFilter = filters.find(
        (filter: Filter) => filter.id === FILTER_RULESET[RulesetType.RULESET_2].id,
    );

    const getRulesMessage = (count: number) => translator.getPlural('options_filter_rules_counter', count, { count });

    return (
        <Category
            navLink="/"
            headerName={translator.getMessage('options_languages_option')}
            headerDesc={translator.getMessage(
                'options_languages_option_header_desc',
            )}
        >
            <>
                ...
                <div className={styles.container}>
                    <SwitcherOption
                        ...
                        message={translator.getMessage(
                            'options_languages_english',
                        )}
                        title={getRulesMessage(
                            integratedFilter?.declarativeRulesCounter || 0,
                        )}
                    />
                    {languagesFilters.map((filter) => (
                        <SwitcherOption
                            ...
                            title={getRulesMessage(
                                filter.declarativeRulesCounter || 0,
                            )}
                        />
                    ))}
                </div>
