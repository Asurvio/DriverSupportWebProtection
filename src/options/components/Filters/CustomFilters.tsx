...
export const CustomFilters = observer(() => {
    ...
    const { openModal, closeModal } = customFilterModalStore;
    ...
    const { disableFilter, enableFilter } = settingsStore;
    ...
    const [urlToSubscribe, setUrlToSubscribe] = useState(
    ...
    const closeModalHandler = () => {
        ...
        if (query.has(QUERY_PARAM_NAMES.TITLE) || query.has(QUERY_PARAM_NAMES.SUBSCRIBE)) {
            ...
            history.push(
                `${history.location.pathname}?${decodeURIComponent(
                    query.toString(),
                )}`,
            );
        }
            ...
            {filtersByGroupId.length > 0 ? (
                ...
                <>
                    <div className={styles.notFound}>
                        <Icon id={IconId.FILTERS_NOT_FOUND} className={styles.notFoundIcon} />
                        {translator.getMessage('options_filters_not_found')}
                    </div>
                </>
            )}
        </div>
